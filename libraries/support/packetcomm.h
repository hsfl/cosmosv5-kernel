/*!
 * \file packetcomm.h
 * \brief COSMOS PacketComm packet structure and serialization.
 * \ingroup cosmos_kernel_network
 * \defgroup cosmos_packetcomm PacketComm
 *
 * Defines the PacketComm packet format: a fixed header (type, node, agent)
 * followed by an arbitrary payload byte-vector. Provides serialise/deserialise
 * helpers and CRC protection. PacketComm is the standard inter-agent datagram
 * format used throughout COSMOS networking.
 */
...
//                V1,
            ...
            //! \brief Bitwise-invert every byte in \p data in place.
            //! \param data Byte vector to invert.
            void Invert(vector<uint8_t>& data);

            //! \brief Compute and store the CRC over the current \c data payload.
            void CalcCRC();

            //! \brief Verify the CRC stored in \c crc against the current \c data payload.
            //! \return true if the CRC matches, false otherwise.
            bool CheckCRC();

            //! \brief Deserialise \c packetized into \c header and \c data.
            //! \param checkcrc If true, verify CRC before accepting; default true.
            //! \return 0 on success, negative error code on failure.
            int32_t Unwrap(bool checkcrc=true);

            //! \brief Deserialise \c packetized into \c header and \c data with header-style selection.
            //! \param checkcrc       If true, verify CRC.
            //! \param minimal_header If true, use the minimal (no chanin/chanout) header layout.
            //! \return 0 on success, negative error code on failure.
            int32_t Unwrap(bool checkcrc, bool minimal_header);

            //! \brief Deserialise a raw (unwrapped) byte buffer in \c packetized.
            //! \param invert   If true, bitwise-invert the buffer before parsing; default false.
            //! \param checkcrc If true, verify CRC; default true.
            //! \return 0 on success, negative error code on failure.
            int32_t RawUnPacketize(bool invert=false, bool checkcrc=true);

            //! \brief Deserialise a raw byte buffer with header-style selection.
            int32_t RawUnPacketize(bool invert, bool checkcrc, bool minimal_header);

            //! \brief Deserialise an ASM-framed packet from \c packetized.
            //! \return true on success.
            bool ASMUnPacketize();

            //! \brief Deserialise an ASM-framed packet with optional CRC check and descrambling.
            //! \param checkcrc   If true, verify CRC.
            //! \param descramble If true, descramble the payload.
            //! \return true on success.
            bool ASMUnPacketize(bool checkcrc, bool descramble);

            //! \brief Deserialise a SLIP-framed packet from \c packetized.
            //! \param checkcrc If true, verify CRC; default true.
            //! \return true on success.
            bool SLIPUnPacketize(bool checkcrc=true);

            //! \brief Deserialise an HDLC-framed packet from \c packetized.
            //! \param checkcrc If true, verify CRC; default true.
            //! \return true on success.
            bool HDLCUnPacketize(bool checkcrc=true);

            //! \brief Deserialise an AX.25-framed packet from \c packetized.
            //! \param checkcrc If true, verify CRC; default true.
            //! \return true on success.
            bool AX25UnPacketize(bool checkcrc=true);

            //! \brief Serialise \c header + \c data into \c wrapped (with CRC).
            //! \return true on success.
            bool Wrap();

            //! \brief Serialise \c header + \c data into \c wrapped, with CRC control.
            //! \param calc_checksum If true, compute and append CRC.
            //! \return true on success.
            bool Wrap(bool calc_checksum);

            //! \brief Serialise \c wrapped into a raw (no framing) \c packetized buffer.
            //! \return true on success.
            bool RawPacketize();

            //! \brief Serialise \c wrapped into an ASM-framed \c packetized buffer.
            //! \return true on success.
            bool ASMPacketize();

            //! \brief Serialise \c wrapped into an ASM-framed buffer, padding to \p packet_wrapped_size.
            //! \param packet_wrapped_size Target total frame size in bytes.
            //! \param scramble            If true, scramble the payload.
            //! \return true on success.
            bool ASMPacketize(uint16_t packet_wrapped_size, bool scramble);

            //! \brief Serialise \c wrapped into an AX.25-framed \c packetized buffer.
            //! \param dest_call  Destination callsign; default "".
            //! \param sour_call  Source callsign; default "".
            //! \param flagcount  Number of HDLC flag bytes; default 2.
            //! \param dest_stat  Destination SSID byte; default 0x60.
            //! \param sour_stat  Source SSID byte; default 0x61.
            //! \param cont       Control field byte; default 0x03 (UI frame).
            //! \param prot       Protocol ID byte; default 0xf0 (no layer 3).
            //! \return true on success.
            ...
            //! \brief Serialise \c wrapped into an HDLC-framed \c packetized buffer.
            //! \param flagcount Number of flag bytes to prepend; default 10.
            //! \return true on success.
            bool HDLCPacketize(uint8_t flagcount=10);

            //! \brief Serialise \c wrapped into a SLIP-framed \c packetized buffer.
            //! \return true on success.
            bool SLIPPacketize();
