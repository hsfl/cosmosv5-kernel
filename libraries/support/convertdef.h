...
/*!
 * \file convertdef.h
 * \brief Coordinate conversion structure definitions (header-only).
 * \ingroup cosmos_kernel_config
 *
 * Defines the C structures (pos_t, att_t, etc.) that carry position and
 * attitude data in multiple coordinate frames simultaneously. These types are
 * shared between the kernel and agent layers; no implementation file exists.
 */
...
#define MSUN 1.9891e30
        //! SI Radius of Earth (meters)
#define REARTHM static_cast<double>(6378137.)
        //! SI Radius of Earth (kilometers)
